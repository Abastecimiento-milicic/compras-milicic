 ...
 * between tabs or refreshing does not re-download the same
 * CSV/JSON data files.
 * 
...
(function () {
  ...
  window.fetchWithCache = async function fetchWithCache(url) {
    try {
      ...
      if (cached && cached.data) {
        console.log(`[cache-utils] Cache HIT for ${url}`);
        return cached.data;
      }
    } catch (_) {
    ...
    console.log(`[cache-utils] Cache MISS for ${url}. Fetching from network...`);
    // Network fetch
    ...
    if (url.includes(".gz")) {
      ...
      if (typeof fflate !== 'undefined') {
        ...
        data = fflate.strFromU8(decompressed);
      } else {
        throw new Error("fflate library not loaded for decompressing .gz");
      }
    } else {
  ...
  window.clearDataCache = async function clearDataCache() {
    try {
      ...
      console.log(`[cache-utils] Cache cleared successfully`);
    } catch (_) {
      // ignore
    }
  };
