...
window.addEventListener("DOMContentLoaded", () => {
  async function loadFromSupabase() {
    console.log("[cumplimiento] Cargando datos desde Supabase (tabla COMPRAS)...");
    try {
      const dbData = await window.fetchTableFromSupabase('COMPRAS');

      if (!dbData.length) {
        showError("La tabla COMPRAS en Supabase está vacía o no se pudo leer.");
        return;
      }
      ...
      if (missing.length) {
        showError("Faltan columnas requeridas en Supabase: " + missing.join(", "));
        return;
      }
      ...
      console.error(err);
      showError("Error cargando desde Supabase: " + (err?.message || err));
    } finally {
      const loader = document.getElementById("loader");
